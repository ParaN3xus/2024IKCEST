...
class Sequence():
    ...
    def _get_images(self, path, dets_frame, dets_uncl_frame, padding='zero'):
        ...
        if self.net_type == 'IBN':
            ...
            img = self.to_tensor(img).to(self.device)  # 转移到 CUDA
        else:
            img = self.to_tensor(Image.open(path).convert("RGB")).to(self.device)  # 转移到 CUDA
        ...
        if self.random_patches:
            random_patches = self._get_random_patches(img).to(self.device)  # 转移到 CUDA
        else:
            random_patches = None
        ...
        for ind, row in dets_frame.iterrows():
            ...
            if self.fixed_aspect_ratio and not (to_pad and self.padding):
                ...
                im = img[:, int(row['bb_top']):int(row['bb_bot']), int(
                    row['bb_left']):int(row['bb_right'])].to(self.device)  # 转移到 CUDA
            ...
            if self.padding and to_pad:
                im = self.pad_bbs(
                    ...
                    top_pad,
                    bot_pad
                ).to(self.device)  # 转移到 CUDA

            # 转换到 PIL，进行变换，然后再转回 CUDA
            im = self.to_pil(im)
        ...
        # 不同网络的尺度调整
        if self.net_type == "IBN":
            res = torch.stack(res, 0).to(self.device) * 255  # 转移到 CUDA
        else:
            res = torch.stack(res, 0).to(self.device)  # 转移到 CUDA

        # 返回结果
        return res, dets, ids, vis, random_patches, img_for_det, conf, label
